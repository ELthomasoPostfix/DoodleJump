...
void windowTest()
{
        ...
        Rect r1 = {{{100.0, 100.0}, {200.0, 100.0},
                    {200.0, 200.0}, {100.0, 200.0}}};
        std::pair<double, double> c1 = CollisionObject::determineAbsoluteCenterOfMass(r1);
        CollisionObject* cObj1 = new CollisionObject(
                c1.first, c1.second,r1);

        Rect r2 = {{{150.0, 150.0}, {250.0, 150.0},
                    {250.0, 250.0}, {150.0, 250.0}}};
        std::pair<double, double> c2 = CollisionObject::determineAbsoluteCenterOfMass(r2);
        CollisionObject* cObj2 = new CollisionObject(
                c2.first, c2.second,r2);

        //cObj2->move(c2.first + 70, c2.second);
        ...
        while (window.isOpen()) {
            ...
            while (window.pollEvent(event)) {
                    ...
                    if (event.type == sf::Event::KeyPressed) {
                        ...
                        if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
                            auto pushback = cObj2->determinePushback(moveDir, *cObj1, 1.0, requireCollision);
                            cObj2->move(cObj2->getPosition().first + pushback.first,
                                        cObj2->getPosition().second + pushback.second);
                            std::cout << "(" << cObj2->getPosition().first << ", " << cObj2->getPosition().second << ")" << std::endl;
                        } else {
                            auto loc = cObj2->getPosition();
                            cObj2->move(loc.first + dirX, loc.second + dirY);
            ...
            sf::Vertex line[] =
                    {
                            sf::Vertex(sf::Vector2f(lineStart.first, lineStart.second)),
